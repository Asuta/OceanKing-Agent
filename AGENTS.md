...
Human messages and committed Agent `kind=handoff` messages immediately supersede only the single target Agent selected by the room's current round-robin order; that run must preserve an interruption snapshot for takeover. The sender and other non-selected room members are not interruption targets. Agent `kind=notify` messages never enqueue, advance, or interrupt another Agent. Non-message room events such as invitations remain queued without preemption.

Public Agent messages have only two protocol kinds: `notify` is a process update that publishes without satisfying delivery or waking another Agent, while `handoff` is an ending message that publishes and wakes the room's next Agent. A Turn may submit any number of handoffs sequentially by opening one public route at a time; only the final handoff without a following route concludes the Turn or establishes its cross-room continuation. `read_no_reply` remains a non-message receipt for an exact inbound message and is how a receiver terminates that chain without public output; recording the receipt does not terminate the current Turn, so the Agent may continue with another room. Cross-room waits are runtime-managed through `turn_handoffs.awaiting_reply`; a resumed task uses `handoff` both to continue delegation and to return a final result to its source room. Do not add model-visible wait tools or poll `read_room_history` inside the sending Turn.

Agent-created Agents are persistent registry entries with independent sessions and have no ordinary destroy flow. A full workspace reset deletes every runtime-created Agent and its private workspace, retaining only `navigator` and `builder`. Database mutation tools commit in their own idempotent SQLite transaction before reporting success; logical invocation keys remain stable across retry Turns, and durable dispatch outbox rows recover room wakeups after a restart. A committed `handoff` enqueues its target room immediately; Turn completion and startup recovery only retry undispatched outbox rows. A final cross-room `handoff` stores its continuation link in the same transaction as the public message. Turn completion records protocol/session state but does not reapply those mutations. `create_agent` is limited to 16 successful creations per Turn, and its committed IDs are immediately available to later `create_room` or `invite_agent` calls in that same Turn. Any enabled Agent connected to a room may invite another Agent; owner authority is still required for removing participants.
