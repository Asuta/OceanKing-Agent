...
Agent-created Agents are permanent registry entries with independent sessions. `create_agent` is transactional with the creating Turn, is limited to 16 successful creations per Turn, and its pending IDs must remain usable by later `create_room` or `invite_agent` calls in that same Turn. Any enabled Agent connected to a room may invite another Agent; owner authority is still required for removing participants.
