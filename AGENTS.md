...
Agent-created Agents are persistent registry entries with independent sessions and have no ordinary destroy flow. A full workspace reset deletes every runtime-created Agent and its private workspace, retaining only `navigator` and `builder`. `create_agent` is transactional with the creating Turn, is limited to 16 successful creations per Turn, and its pending IDs must remain usable by later `create_room` or `invite_agent` calls in that same Turn. Any enabled Agent connected to a room may invite another Agent; owner authority is still required for removing participants.
