...
- Agent 必须调用 `begin_message_to_room` 打开公开输出通道，后续 Assistant 正文才会进入房间。
- Agent 可调用 `create_agent` 创建持续存在的新 Agent，指定名称、简介和系统提示词；每个 Turn 最多创建 16 个，返回的 ID 可在同一 Turn 用于建房或邀请。执行“数据重置”时，运行期创建的 Agent 及其私有工作区会被删除，只保留最初两个 Agent。
- Agent 调用 `create_room` 时会自动成为新房间 owner 并连接，可在同一原子操作中通过 `agentIds` 邀请多个可用 Agent。
- 任意已连接房间的 Agent 都可调用 `invite_agent` 邀请成员，不要求 owner；移除成员仍需要 owner 权限。
- Agent 公开消息只有 `notify` 和 `handoff` 两种：`notify` 是过程消息，仅公开内容，当前 Agent 继续执行，不完成本轮交付，也不触发其他 Agent；`handoff` 是结束消息，公开内容并触发房间中的下一个 Agent，同一 Turn 可以依次向多个房间发送 handoff。接收方确实无需回复时使用 `read_no_reply` 结清精确消息；它不产生消息气泡、不结束当前 Turn，也不属于消息类型。
- 公开正文生成期间会通过 SSE 显示临时流式气泡；只有正文成功提交后才写入正式消息，失败或停止时不会留下半截内容。
- 用户消息和 Agent 的 `handoff` 消息具有相同的抢占权：消息写入后会立即中止目标 Agent 的旧 run，保存未完成快照，并由包含新旧消息的新 Turn 接管；`notify` 不抢占、不排队，也不推进其他 Agent。
