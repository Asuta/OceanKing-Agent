...
- Agent 公开消息只有 `notify` 和 `handoff` 两种：`notify` 是过程消息，仅公开内容，当前 Agent 继续执行，不完成本轮交付，也不触发其他 Agent；`handoff` 是结束消息，公开内容、结束当前 Turn，并触发房间中的下一个 Agent。接收方确实无需回复时使用 `read_no_reply` 终止，它是精确消息的处理动作，不属于消息类型。
