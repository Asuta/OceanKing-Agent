...
export function CronDrawer({ snapshot, roomId, allowedAgentIds, unavailableReason, busy, sendCommand, onClose }: { snapshot: WorkspaceSnapshot; roomId: string; allowedAgentIds?: string[]; unavailableReason?: string; busy: boolean; sendCommand: SendCommand; onClose: () => void }) {
  const [creating, setCreating] = useState(false);
  const availableAgents = useMemo(() => allowedAgentIds ? snapshot.agents.filter((agent) => allowedAgentIds.includes(agent.id)) : snapshot.agents, [allowedAgentIds, snapshot.agents]);
  const [draft, setDraft] = useState({ name: "每日跟进", schedule: "0 9 * * *", timezone: "Asia/Shanghai", prompt: "检查这个房间中的未完成事项，并通过房间工具公开进展。", agentId: availableAgents[0]?.id ?? "" });
  const jobs = useMemo(() => roomId ? snapshot.cronJobs.filter((job) => job.roomId === roomId) : [], [roomId, snapshot.cronJobs]);
