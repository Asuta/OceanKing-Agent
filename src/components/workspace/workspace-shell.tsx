  ...
  return (
    <main className={`workspace-shell ${consoleOpen ? "console-visible" : ""}`}>
      ...
      <section className={`room-stage ${mobileView !== "room" ? "mobile-hidden" : ""}`}>
        ...
        {room ? <RoomPanel room={room} agents={workspace.snapshot.agents} busy={workspace.busy} sendCommand={workspace.sendCommand} onToggleConsole={() => setConsoleOpen((value) => !value)} consoleOpen={consoleOpen} /> : <div className="empty-stage">创建一个房间开始协作</div>}
      </section>
