...
export const interruptedTurnSystemInstructions = [
  ...
  "结束当前轮之前，检查是否仍欠任何房间承诺过的答复、跟进、工具结果或进度说明；若未被明确取消，就完成它，或先通过 begin_message_to_room 打开正确的原房间，再用下一次 assistant 正文发送可见进度。",
] as const;
...
export function buildInterruptedTurnSnapshot(args: {
  ...
  sections.push(
    [
      ...
      "结束当前轮之前，要么完成原房间义务，要么先通过 begin_message_to_room 打开正确的原房间，再用下一次 assistant 正文发送可见进度。",
    ].join("\n"),
