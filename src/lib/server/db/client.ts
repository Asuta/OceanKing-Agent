...
CREATE TABLE IF NOT EXISTS turn_handoffs (source_turn_id TEXT PRIMARY KEY REFERENCES agent_turns(id) ON DELETE CASCADE, agent_id TEXT NOT NULL, source_room_id TEXT NOT NULL REFERENCES rooms(id) ON DELETE CASCADE, source_participant_id TEXT NOT NULL, cutoff_seq INTEGER NOT NULL, target_room_id TEXT NOT NULL REFERENCES rooms(id) ON DELETE CASCADE, target_turn_id TEXT REFERENCES agent_turns(id) ON DELETE SET NULL, delivery_only INTEGER NOT NULL DEFAULT 0, deferred INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL);
...
export function createDatabase(explicitDataDir?: string): DatabaseHandle {
  ...
  if (!handoffColumns.has("deferred")) raw.exec("ALTER TABLE turn_handoffs ADD COLUMN deferred INTEGER NOT NULL DEFAULT 0");
