...
CREATE TABLE IF NOT EXISTS rooms (id TEXT PRIMARY KEY, title TEXT NOT NULL, kind TEXT NOT NULL DEFAULT 'shared', direct_agent_id TEXT REFERENCES agents(id) ON DELETE CASCADE, owner_participant_id TEXT, next_seq INTEGER NOT NULL DEFAULT 1, archived_at TEXT, pinned_at TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
...
CREATE TABLE IF NOT EXISTS turn_handoffs (source_turn_id TEXT PRIMARY KEY REFERENCES agent_turns(id) ON DELETE CASCADE, agent_id TEXT NOT NULL, source_room_id TEXT NOT NULL REFERENCES rooms(id) ON DELETE CASCADE, source_participant_id TEXT NOT NULL, cutoff_seq INTEGER NOT NULL, target_room_id TEXT NOT NULL REFERENCES rooms(id) ON DELETE CASCADE, target_turn_id TEXT REFERENCES agent_turns(id) ON DELETE SET NULL, delivery_only INTEGER NOT NULL DEFAULT 0, awaiting_reply INTEGER NOT NULL DEFAULT 0, awaiting_message_id TEXT REFERENCES room_messages(id) ON DELETE SET NULL, created_at TEXT NOT NULL);
...
export function createDatabase(explicitDataDir?: string): DatabaseHandle {
  ...
  if (!roomColumns.has("kind")) raw.exec("ALTER TABLE rooms ADD COLUMN kind TEXT NOT NULL DEFAULT 'shared'");
  if (!roomColumns.has("direct_agent_id")) raw.exec("ALTER TABLE rooms ADD COLUMN direct_agent_id TEXT REFERENCES agents(id) ON DELETE CASCADE");
  raw.exec("CREATE UNIQUE INDEX IF NOT EXISTS rooms_direct_agent_unique ON rooms(direct_agent_id) WHERE kind='direct' AND direct_agent_id IS NOT NULL");
  ...
  if (!handoffColumns.has("awaiting_message_id")) raw.exec("ALTER TABLE turn_handoffs ADD COLUMN awaiting_message_id TEXT REFERENCES room_messages(id) ON DELETE SET NULL");
