...
const tools: ToolDefinition[] = [
  ...
  {
    name: "create_room", description: "创建一个由当前 Agent 拥有的新房间。", schema: z.object({ title: z.string().trim().min(1).max(120) }),
    parameters: { type: "object", additionalProperties: false, required: ["title"], properties: { title: { type: "string" } } },
    execute: async (_context, raw) => { const args = z.object({ title: z.string().trim().min(1).max(120) }).parse(raw); const effect: TurnEffect = { type: "create_room", roomId: createId("room"), title: args.title }; return { text: `新房间 ID：${effect.roomId}`, structured: effect, effects: [effect] }; },
  },
  {
    name: "invite_agent", description: "房间 owner 邀请一个 Agent。", schema: z.object({ roomId: z.string(), agentId: z.string() }),
    ...
    execute: async (context, raw) => { const args = z.object({ roomId: z.string(), agentId: z.string() }).parse(raw); const room = requireConnectedRoom(context, args.roomId); if (room.ownerParticipantId !== context.agentParticipantId) throw new Error("只有 owner Agent 能邀请成员"); if (!context.repository.getAgent(args.agentId)) throw new Error("Agent 不存在"); const effect: TurnEffect = { type: "invite_agent", roomId: args.roomId, agentId: args.agentId, participantId: createId("participant") }; return { text: "邀请动作已准备提交", structured: effect, effects: [effect] }; },
  },
  {
    ...
    execute: async (context, raw) => { const args = z.object({ roomId: z.string(), participantId: z.string() }).parse(raw); const room = requireConnectedRoom(context, args.roomId); if (room.ownerParticipantId !== context.agentParticipantId) throw new Error("只有 owner Agent 能移除成员"); const effect: TurnEffect = { type: "remove_participant", ...args }; return { text: "移除动作已准备提交", structured: effect, effects: [effect] }; },
  },
  {
