...
const tools: ToolDefinition[] = [
  ...
  },
  {
    ...
    execute: async (context, raw, callId, invocationKey) => { const args = inviteAgentToolSchema.parse(raw); requireSharedRoom(context, args.roomId); if (!hasAvailableAgent(context, args.agentId)) throw new Error("Agent 不存在"); const effect = commitEffect(context, "invite_agent", callId, invocationKey, { type: "invite_agent", roomId: args.roomId, agentId: args.agentId, participantId: createId("participant") }); return { text: "邀请已提交并立即生效", structured: effect, effects: [effect] }; },
  },
  {
  ...
  },
  {
    ...
    execute: async (context, raw, callId, invocationKey) => { const args = z.object({ roomId: z.string() }).parse(raw); requireSharedRoom(context, args.roomId); const participant = context.repository.getRoom(args.roomId)?.participants.find((p) => p.agentId === context.agent.id); if (!participant) throw new Error("成员不存在"); const effect = commitEffect(context, "leave_room", callId, invocationKey, { type: "leave_room", roomId: args.roomId, participantId: participant.id }); return { text: "已离开房间", structured: effect, effects: [effect] }; },
  },
